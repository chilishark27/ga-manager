        ...
        <div className="page-card" style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
            ...
            <div className="token-stat-item">
              ...
              <span className="token-stat-value">{costStats?.input != null ? `${(costStats.input / 1000).toFixed(1)}K` : (tokenStats?.input_tokens ? `${(tokenStats.input_tokens / 1000).toFixed(1)}K` : '0')}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">Output</span>
              <span className="token-stat-value">{costStats?.output != null ? `${(costStats.output / 1000).toFixed(1)}K` : (tokenStats?.output_tokens ? `${(tokenStats.output_tokens / 1000).toFixed(1)}K` : '0')}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">{lang === 'zh' ? '缓存命中' : 'Cache Hit'}</span>
              <span className="token-stat-value">{costStats?.cache_hit_rate != null ? `${costStats.cache_hit_rate}%` : (tokenStats?.cache_hit_rate ? `${tokenStats.cache_hit_rate.toFixed(1)}%` : '0%')}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">Cache Read</span>
              <span className="token-stat-value">{costStats?.cache_read != null ? `${(costStats.cache_read / 1000).toFixed(1)}K` : (tokenStats?.cache_read ? `${(tokenStats.cache_read / 1000).toFixed(1)}K` : '0')}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">Turns</span>
              <span className="token-stat-value">{costStats?.requests ?? tokenStats?.total_turns ?? 0}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">{lang === 'zh' ? '会话时长' : 'Duration'}</span>
              <span className="token-stat-value">{costStats?.elapsed_seconds ? `${Math.floor(costStats.elapsed_seconds / 60)}m` : '0m'}</span>
            </div>
            <div className="token-stat-item">
              <span className="token-stat-label">{lang === 'zh' ? '总计' : 'Total'}</span>
              <span className="token-stat-value">{costStats?.total_tokens != null ? `${(costStats.total_tokens / 1000).toFixed(1)}K` : '0'}</span>
            </div>
          </div>
        </div>
