          ...
          {subagents.map(sa => {
            ...
            >
              ...
              <div className="conductor-agent-actions" onClick={e => e.stopPropagation()}>
                ...
                <button className="conductor-action-btn" style={{ borderColor: 'rgba(255,77,79,0.3)', color: 'var(--red)' }} onClick={() => { doAction(sa.id, 'kill'); setSubagents(prev => prev.filter(s => s.id !== sa.id)); }}>Del</button>
              </div>
            </div>
            );
          })}
